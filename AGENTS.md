...
- **ArcImportService** - Parses Arc export JSON into `ImportWorkspace` values; emits workspace emoji icons (`customIcon`), inserts `Separator` nodes between Arc pinned/unpinned groups, and is the canonical example of the build-then-replace import pattern
- **ChromeImportService** - Parses Netscape bookmark files (Chrome/Safari/Firefox exports) into an `ImportWorkspace`
- **TabbitImportService** - Parses the latest Chromium `Session_*` file from the Tabbit profile into an `ImportWorkspace` (group metadata from cmd 27, tab-to-group binding from cmd 25, tab order from cmd 2, selected navigation from cmd 7, navigation records from cmd 6)

### Fork-specific architecture (beyond upstream v0.1.14)

These subsystems are specific to this fork. See `docs/FORK_CHANGES.md` for the full commit-by-commit record.

**iCloud readonly sync (sync roles)**
- `SyncRole` in `Constants.swift` (`primary` / `secondary`, default `.secondary`) is the source of truth for whether this device may write shared data. Persisted in UserDefaults; posts `.syncRoleChanged`.
- `DataStore.canWriteSharedData = !usesICloudDirectory || syncRole == .primary`. When false, the store never creates/overwrites the shared `data.json`, never creates directories in the shared location, and routes writes-for-locals (icons, agent endpoint) to `localBaseDirectory`.
- On iCloud-backed directories, `DataStore` triggers `startDownloadingUbiquitousItem` + polls download status (up to ~3s), reads via `NSFileCoordinator`, and on first launch migrates local `data.json`/`Icons`/`Notes` into iCloud once.
- `AppModel` guards every shared-data mutation with `guardCanWriteSharedData()` and exposes `reloadFromStoreIfChanged()` (reloads from disk, restoring local-only selection: which workspace is active and whether Settings is open). Local-only selection is stripped before persistence via `stateForPersistence()`.
- `NoteStorage.write` throws `.readOnly` in secondary mode; `NoteServer` maps that to HTTP 403.

**Workspace overwrite / import flow**
- `AppModel.replaceAllWorkspaces(with:)` atomically replaces the entire workspace list (used by Arc and Tabbit import). Re-import replaces content instead of merging.
- `AppModel.createWorkspace(name:colorId:items:)` accepts an optional `items:` array so importers can build a workspace in one shot. Importers should prefer building `Workspace` values directly and using `replaceAllWorkspaces` / `createWorkspace(items:)` over the older "create then mutate selected workspace" pattern.
- In Settings, `applyImport` (Arc) and `applyTabbitImport` build `Workspace` values and call `replaceAllWorkspaces`; `applyChromeImport` uses `createWorkspace(items:)`. All three respect the shared-data write gate and show an error in readonly mode.

**Node types**
- `Node` has a fourth case, `.separator(Separator)`, used by Arc import to visually divide pinned and unpinned groups. Separators are fully wired into the recursive tree operations (`insertNode`, `updateNode`, `removeNode`, `findNodeLocation`, `nodeById`, `containsNode`) and are skipped by rename/cleanup paths.

**Workspace custom icons**
- `Workspace.customIcon: CustomIcon?` (`.emoji` / `.sfSymbol` / `.favicon`). Populated by Arc import from `customInfo.iconType.emoji_v2` (with a fallback to the legacy scalar `emoji`). Rendered in the workspace switcher, Settings workspace list, and workspace chips via `EmojiIconView`.
